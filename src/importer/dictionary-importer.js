...
import {
  getFileExtensionFromImageMediaType,
  getImageMediaTypeFromFileName,
} from "../media/media-util.js";
import { compareRevisions } from "./dictionary-data-util.js";

const ajvSchemas =
  /** @type {import('dictionary-importer').CompiledSchemaValidators} */ (
    /** @type {unknown} */ (ajvSchemas0)
  );
const BlobWriter = /** @type {typeof import('@zip.js/zip.js').BlobWriter} */ (
  /** @type {unknown} */ (BlobWriter0)
);
const TextWriter = /** @type {typeof import('@zip.js/zip.js').TextWriter} */ (
  /** @type {unknown} */ (TextWriter0)
);
const Uint8ArrayReader =
  /** @type {typeof import('@zip.js/zip.js').Uint8ArrayReader} */ (
    /** @type {unknown} */ (Uint8ArrayReader0)
  );
const ZipReader = /** @type {typeof import('@zip.js/zip.js').ZipReader} */ (
  /** @type {unknown} */ (ZipReader0)
);
...
export class DictionaryImporter {
  ...
  /**
   * @param {import('../database/sqlite-adapter.js').SQLiteAdapter} dictionaryDatabase
  ...
  async importDictionary(dictionaryDatabase, archiveContent, details) {
    ...
    const bulkAdd = async (objectStoreName, entries) => {
      ...
      for (let i = 0; i < entryCount; i += maxTransactionLength) {
        ...
        try {
          dictionaryDatabase.bulkAdd(objectStoreName, entries, i, count);
        } catch (e) {
    ...
    const version = /** @type {import('dictionary-data').IndexVersion} */ (
      index.version
    );

    // Verify database is not already imported
    if (dictionaryDatabase.dictionaryExists(dictionaryTitle)) {
      return {
        errors: [
          new Error(
            `Dictionary ${dictionaryTitle} is already imported, skipped it.`,
          ),
        ],
        result: null,
      };
    ...
    const { termFiles, termMetaFiles, kanjiFiles, kanjiMetaFiles, tagFiles } =
      Object.fromEntries(this._getArchiveFiles(fileMap, queryDetails));

    console.log("termFiles:" + termFiles);
    console.log("termMetaFiles:" + termMetaFiles);
    console.log("kanjiFiles:" + kanjiFiles);
    console.log("kanjiMetaFiles:" + kanjiMetaFiles);
    ...
    let summary = this._createSummary(
      dictionaryTitle,
      version,
      index,
      summaryDetails,
    );
    // SQLite adapter returns the ID directly (not an IDBRequest)
    const primaryKey = /** @type {number} */ (dictionaryDatabase.addWithResult(
      "dictionaries",
      summary,
    ));
    ...
    try {
      ...
      for (const termFile of termFiles) {
        ...
        // let { media } = await this._resolveAsyncRequirements(
      ...
      for (const termMetaFile of termMetaFiles) {
        let termMetaList = await this._readFileSequence(
          [termMetaFile],
          this._convertTermMetaBankEntry.bind(this),
          dictionaryTitle,
        );

        await bulkAdd("termMeta", termMetaList);
        for (const [key, value] of Object.entries(
          this._getMetaCounts(termMetaList),
        )) {
      ...
      for (const kanjiMetaFile of kanjiMetaFiles) {
        let kanjiMetaList = await this._readFileSequence(
          [kanjiMetaFile],
          this._convertKanjiMetaBankEntry.bind(this),
          dictionaryTitle,
        );

        await bulkAdd("kanjiMeta", kanjiMetaList);
        for (const [key, value] of Object.entries(
          this._getMetaCounts(kanjiMetaList),
        )) {
      ...
      for (const tagFile of tagFiles) {
        let tagList = await this._readFileSequence(
          [tagFile],
    ...
    dictionaryDatabase.bulkUpdate(
      "dictionaries",
      [{ data: summary, primaryKey }],
      0,
      1,
    );
  ...
  _findRedundantDirectories(fileMap) {
    ...
    const redundantDirectoriesRegex = new RegExp(
      `.*(?=${INDEX_FILE_NAME.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")})`,
    );
  ...
  async _readAndValidateIndex(fileMap) {
    ...
    if (typeof indexFile === "undefined") {
      ...
      if (redundantDirectories) {
        throw new Error(
          'Dictionary index found nested in redundant directories: "' +
            redundantDirectories +
            "\" when it must be in the archive's root directory",
        );
      }
      throw new Error("No dictionary index found in archive");
    }
    const indexFile2 = /** @type {import('@zip.js/zip.js').Entry} */ (
      indexFile
    );
    ...
    if (!ajvSchemas.dictionaryIndex(index)) {
      throw this._formatAjvSchemaError(
        ajvSchemas.dictionaryIndex,
        INDEX_FILE_NAME,
      );
    }
    ...
    const version =
      typeof validIndex.format === "number"
        ? validIndex.format
        : validIndex.version;
  ...
  _createSummary(dictionaryTitle, version, index, details) {
    ...
    if (typeof minimumYomitanVersion === "string") {
      if (details.yomitanVersion === "0.0.0.0") {
        ...
        compareRevisions(details.yomitanVersion, minimumYomitanVersion)
      ) {
        throw new Error(
    ...
    if (typeof isUpdatable === "boolean") {
      ...
      if (
        !isUpdatable ||
        !this._validateUrl(indexUrl) ||
        !this._validateUrl(downloadUrl)
      ) {
  ...
  _formatAjvSchemaError(schema, fileName) {
    const e = new ExtensionError(
      `Dictionary has invalid data in '${fileName}' '${JSON.stringify(schema.errors)}'`,
    );
  ...
  _formatDictionaryTermGlossaryObject(data, entry, requirements) {
    switch (data.type) {
      case "text":
        return data.text;
      case "image":
        return this._formatDictionaryTermGlossaryImage(
          data,
          entry,
          requirements,
        );
      ...
      default:
        throw new Error(
          `Unhandled data type: ${/** @type {import('core').SerializableObject} */ (data).type}`,
        );
    }
  }
  ...
  _formatStructuredContent(data, entry, requirements) {
    const content = this._prepareStructuredContent(
      data.content,
      entry,
      requirements,
    );
  ...
  _prepareStructuredContent(content, entry, requirements) {
    ...
    if (Array.isArray(content)) {
      for (let i = 0, ii = content.length; i < ii; ++i) {
        content[i] = this._prepareStructuredContent(
          content[i],
          entry,
          requirements,
        );
    ...
    if (typeof childContent !== "undefined") {
      content.content = this._prepareStructuredContent(
        childContent,
        entry,
        requirements,
      );
  ...
  async _getImageMedia(context, path, entry) {
    ...
    // Load file content
    let content = await (
      await this._getData(file, new BlobWriter())
    ).arrayBuffer();
    ...
    // Load image data
    // TODO: Implement image dimension detection for server-side
    // For now, use placeholder dimensions since media loading is disabled
    const width = 0;
    const height = 0;
    // let width;
    // let height;
    // try {
    //   ({ content, width, height } = await this._mediaLoader.getImageDetails(
    //     content,
    //     mediaType,
    //   ));
    // } catch (e) {
  ...
  _convertTermBankEntryV1(entry, dictionary) {
    let [expression, reading, definitionTags, rules, score, ...glossary] =
      entry;
  ...
  _convertTermMetaBankEntry(entry, dictionary) {
    ...
    return /** @type {import('dictionary-database').DatabaseTermMeta} */ ({
      expression,
      mode,
      data,
      dictionary,
  ...
  async _getData(entry, writer) {
    // Entry is DirectoryEntry | FileEntry - only FileEntry has getData
    if (entry.directory) {
      throw new Error(`Cannot read directory ${entry.filename}`);
    }
