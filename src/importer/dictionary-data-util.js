...
export function groupTermFrequencies(dictionaryEntry, dictionaryInfo) {
  ...
  for (const [dictionary, map2] of map1.entries()) {
    ...
    const currentDictionaryInfo = dictionaryInfo.find(
...
export function getGroupedPronunciations(dictionaryEntry) {
  ...
  for (const {
    ...
    dictionaryAlias,
    pronunciations,
  } of termPronunciations) {
    const { term, reading } = headwords[headwordIndex];
    let dictionaryGroupedPronunciationList =
      groupedPronunciationsMap.get(dictionary);
    if (typeof dictionaryGroupedPronunciationList === "undefined") {
      dictionaryGroupedPronunciationList = [];
      groupedPronunciationsMap.set(
        dictionary,
        dictionaryGroupedPronunciationList,
      );
      aliasMap.set(dictionary, dictionaryAlias);
  ...
  for (const [
    dictionary,
    dictionaryGroupedPronunciationList,
  ] of groupedPronunciationsMap.entries()) {
    ...
    for (const groupedPronunciation of dictionaryGroupedPronunciationList) {
      ...
      const exclusiveTerms = !areSetsEqual(terms, allTerms)
...
function findExistingGroupedPronunciation(
  reading,
  pronunciation,
  groupedPronunciationList,
) {
  const existingGroupedPronunciation = groupedPronunciationList.find(
    (groupedPronunciation) => {
      return (
        groupedPronunciation.reading === reading &&
        arePronunciationsEquivalent(groupedPronunciation, pronunciation)
      );
    },
  );
...
function arePronunciationsEquivalent(
  ...
  switch (pronunciation1.type) {
    case "pitch-accent": {
      ...
      const pitchAccent2 = /** @type {import('dictionary').PitchAccent} */ (
        pronunciation2
      );
      return (
        pronunciation1.positions === pitchAccent2.positions &&
        areArraysEqual(
          pronunciation1.nasalPositions,
          pitchAccent2.nasalPositions,
        ) &&
        areArraysEqual(
          pronunciation1.devoicePositions,
          pitchAccent2.devoicePositions,
        )
      );
    }
    case "phonetic-transcription": {
      // This cast is valid based on the type check at the start of the function.
      const phoneticTranscription2 =
...
function areTagListsEqual(tagList1, tagList2) {
  ...
  for (let i = 0; i < ii; ++i) {
    ...
    if (
      tag1.name !== tag2.name ||
      !areArraysEqual(tag1.dictionaries, tag2.dictionaries)
    ) {
      return false;
    }
  }
