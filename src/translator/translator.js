...
import { applyTextReplacement } from "../lib/regex-util.js";
...
export class Translator {
  /**
   * @param {import('../database/sqlite-adapter.js').SQLiteAdapter} database
   */
  constructor(database) {
    /** @type {import('../database/sqlite-adapter.js').SQLiteAdapter} */
  ...
  prepare() {
    ...
    for (const {
      ...
      textPreprocessors = [],
      textPostprocessors = [],
    } of getAllLanguageTextProcessors()) {
      this._textProcessors.set(iso, { textPreprocessors, textPostprocessors });
    }
    for (const {
      iso,
      readingNormalizer,
    } of getAllLanguageReadingNormalizers()) {
  ...
  async findTerms(mode, text, options) {
    ...
    let { dictionaryEntries, originalTextLength } =
      await this._findTermsInternal(
        text,
        options,
        tagAggregator,
        primaryReading,
    ...
    switch (mode) {
      ...
      case "term":
        ...
        break;
      case "merge":
        dictionaryEntries = await this._getRelatedDictionaryEntries(
          dictionaryEntries,
          options,
          tagAggregator,
        );
    ...
    if (excludeDictionaryDefinitions !== null) {
      this._removeExcludedDefinitions(
        dictionaryEntries,
        excludeDictionaryDefinitions,
      );
    }
    ...
    if (mode !== "simple") {
      await this._addTermMeta(
        dictionaryEntries,
        enabledDictionaryMap,
        tagAggregator,
      );
      await this._expandTagGroupsAndGroup(
        tagAggregator.getTagExpansionTargets(),
      );
    } else {
    ...
    const withUserFacingInflections = this._addUserFacingInflections(
      language,
      dictionaryEntries,
    );
  ...
  async _findTermsInternal(text, options, tagAggregator, primaryReading) {
    ...
    if (
      ...
      ["ja", "zh", "yue", "ko"].includes(options.language)
    ) {
    ...
    return this._getDictionaryEntries(
      deinflections,
      enabledDictionaryMap,
      tagAggregator,
      primaryReading,
  ...
  _getDictionaryEntries(
    ...
    for (const {
      ...
      for (const databaseEntry of databaseEntries) {
        ...
        if (ids.has(id)) {
          const existingEntryInfo = this._findExistingEntry(
            dictionaryEntries,
            id,
          );
          ...
          const existingTransformedLength =
            existingEntry.headwords[0].sources[0].transformedText.length;
          ...
          if (transformedText.length > existingTransformedLength) {
            ...
            this._mergeInflectionRuleChains(
              existingEntry,
              inflectionRuleChainCandidates,
            );
            this._mergeTextProcessorRuleChains(
              existingEntry,
              textProcessorRuleChainCandidates,
            );
          }
  ...
  async _getDeinflections(text, options) {
    ...
    await this._addEntriesToDeinflections(
      language,
      deinflections,
      enabledDictionaryMap,
      matchType,
    ...
    for (const deinflection of deinflections) {
      ...
      deinflection.databaseEntries = deinflection.databaseEntries.filter(
        (entry) => entry.definitions.length,
      );
    }
    deinflections = deinflections.filter(
      (deinflection) => deinflection.databaseEntries.length,
    );
   ...
   * @returns {Promise<import('translation-internal').DatabaseDeinflection[]>}
   */
  async _getDictionaryDeinflections(
    language,
    deinflections,
    enabledDictionaryMap,
    matchType,
          ...
          if (Array.isArray(definition)) {
            ...
            const inflectionRuleChainCandidates = algorithmChains.map(
              ({ inflectionRules: algInflections }) => {
                return {
                  source: /** @type {import('dictionary').InflectionSource} */ (
                    algInflections.length === 0 ? "dictionary" : "both"
                  ),
   ...
   * @param {import('dictionary').TermSourceMatchType} matchType
   */
  async _addEntriesToDeinflections(
    language,
    deinflections,
    enabledDictionaryMap,
    matchType,
    ...
    const uniqueDeinflectionsMap =
      this._groupDeinflectionsByTerm(deinflections);
    const uniqueDeinflectionArrays = [...uniqueDeinflectionsMap.values()];
    const uniqueDeinflectionTerms = [...uniqueDeinflectionsMap.keys()];

    const databaseEntries = this._database.findTermsBulk(
      uniqueDeinflectionTerms,
      enabledDictionaryMap,
      matchType,
    );
    this._matchEntriesToDeinflections(
      language,
      databaseEntries,
      uniqueDeinflectionArrays,
      enabledDictionaryMap,
    );
  }
  ...
  _matchEntriesToDeinflections(
    ...
    for (const databaseEntry of databaseEntries) {
      const entryDictionary =
        /** @type {import('translation').FindTermDictionary} */ (
          enabledDictionaryMap.get(databaseEntry.dictionary)
        );
      const { partsOfSpeechFilter } = entryDictionary;

      const definitionConditions =
        this._multiLanguageTransformer.getConditionFlagsFromPartsOfSpeech(
          language,
          databaseEntry.rules,
        );
      for (const deinflection of uniqueDeinflectionArrays[
        databaseEntry.index
      ]) {
        if (
          !partsOfSpeechFilter ||
  ...
  _getAlgorithmDeinflections(text, options) {
    ...
    for (
      ...
      for (const [
        source,
        preprocessorRuleChainCandidates,
      ] of preprocessedTextVariants) {
        for (const deinflection of this._multiLanguageTransformer.transform(
          language,
          source,
        )) {
          ...
          for (const [
            transformedText,
            postprocessorRuleChainCandidates,
          ] of postprocessedTextVariants) {
            ...
            const textProcessorRuleChainCandidates =
              preprocessorRuleChainCandidates.flatMap(
                (preprocessorRuleChainCandidate) =>
                  postprocessorRuleChainCandidates.map(
  ...
  _getTextVariants(text, textProcessors, textReplacements, textCache) {
    ...
    for (const {
      ...
      for (const [
        variant,
        currentPreprocessorRuleChainCandidates,
      ] of variantsMap) {
        for (const option of options) {
          const processed = this._getProcessedText(
            textCache,
            variant,
            id,
            option,
          ...
          if (processed === variant) {
            ...
            newVariantsMap.set(
              processed,
              currentPreprocessorRuleChainCandidates.map((candidate) => [
                ...candidate,
                id,
              ]),
            );
   ...
   */
  async _getRelatedDictionaryEntries(
    dictionaryEntries,
    options,
    tagAggregator,
  ) {
    const { mainDictionary, enabledDictionaryMap, language, primaryReading } =
      options;
    ...
    if (sequenceList.length > 0) {
      const secondarySearchDictionaryMap =
        this._getSecondarySearchDictionaryMap(enabledDictionaryMap);
      ...
      if (
        ungroupedDictionaryEntriesMap.size > 0 ||
        secondarySearchDictionaryMap.size > 0
      ) {
   ...
   * @param {string} primaryReading
   */
  async _addRelatedDictionaryEntries(
    groupedDictionaryEntries,
    ungroupedDictionaryEntriesMap,
    sequenceList,
    enabledDictionaryMap,
    ...
    const databaseEntries =
      await this._database.findTermsBySequenceBulk(sequenceList);
    for (const databaseEntry of databaseEntries) {
      const { dictionaryEntries, ids } =
        groupedDictionaryEntries[databaseEntry.index];
      const { id } = databaseEntry;
      if (ids.has(id)) {
        continue;
      }
   ...
   * @param {string} primaryReading
   */
  async _addSecondaryRelatedDictionaryEntries(
    language,
    groupedDictionaryEntries,
    ungroupedDictionaryEntriesMap,
    enabledDictionaryMap,
    ...
    for (const group of groupedDictionaryEntries) {
      ...
      for (const dictionaryEntry of dictionaryEntries) {
        ...
        const normalizedReading =
          typeof readingNormalizer === "undefined"
            ? reading
    ...
    for (const [
      id,
      dictionaryEntry,
    ] of ungroupedDictionaryEntriesMap.entries()) {
      const { term, reading } = dictionaryEntry.headwords[0];
      const normalizedReading =
        typeof readingNormalizer === "undefined"
          ? reading
    ...
    const databaseEntries = await this._database.findTermsExactBulk(
      termList,
      secondarySearchDictionaryMap,
    );
  ...
  _removeExcludedDefinitions(dictionaryEntries, excludeDictionaryDefinitions) {
    for (let i = dictionaryEntries.length - 1; i >= 0; --i) {
      ...
      const { definitions, pronunciations, frequencies, headwords } =
        dictionaryEntry;
      const definitionsChanged = this._removeArrayItemsWithDictionary(
        definitions,
        excludeDictionaryDefinitions,
      );
      this._removeArrayItemsWithDictionary(
        pronunciations,
        excludeDictionaryDefinitions,
      );
      this._removeArrayItemsWithDictionary(
      ...
      this._removeTagGroupsWithDictionary(
        definitions,
        excludeDictionaryDefinitions,
      );
      this._removeTagGroupsWithDictionary(
  ...
  async _expandTagGroups(tagTargets) {
    ...
    /** @type {import('translator').TagTargetItem[]} */
    ...
    if (nonCachedItemCount > 0) {
      const databaseTags = this._database.findTagMetaBulk(nonCachedItems);
      for (let i = 0; i < nonCachedItemCount; ++i) {
        const item = nonCachedItems[i];
        const databaseTag = databaseTags[i];
        const databaseTag2 =
          typeof databaseTag !== "undefined" ? databaseTag : null;
  ...
  _flagRedundantDefinitionTags(definitions) {
    ...
    for (const { dictionary, tags } of definitions) {
      const partOfSpeech = this._createMapKey(
        this._getTagNamesWithCategory(tags, "partOfSpeech"),
      );
  ...
  async _addTermMeta(dictionaryEntries, enabledDictionaryMap, tagAggregator) {
    ...
    const metas = this._database.findTermMetaBulk(
      headwordMapKeys,
      enabledDictionaryMap,
    );
    for (const { mode, data, dictionary, index } of metas) {
      const { index: dictionaryIndex } = this._getDictionaryOrder(
        dictionary,
        enabledDictionaryMap,
      );
      const dictionaryAlias = this._getDictionaryAlias(
        dictionary,
        enabledDictionaryMap,
      );
          ...
          case "freq":
            {
              ...
              const frequency = hasReading
                ? data.frequency
                : /** @type {import('dictionary-data').GenericFrequencyData} */ (
                    data
                  );
  ...
  _getDictionaryOrder(dictionary, enabledDictionaryMap) {
    ...
    const { index } =
      typeof info !== "undefined" ? info : { index: enabledDictionaryMap.size };
    return { index };
  }
  ...
  _createTermDictionaryEntryFromDatabaseEntry(
    ...
    const matchPrimaryReading =
      primaryReading.length > 0 && reading === primaryReading;
    const { index: dictionaryIndex } = this._getDictionaryOrder(
      dictionary,
      enabledDictionaryMap,
    );
    ...
    const sourceTermExactMatchCount =
      isPrimary && deinflectedText === term ? 1 : 0;
  ...
  _createGroupedDictionaryEntry(
    ...
    for (const dictionaryEntry of dictionaryEntries) {
      ...
      for (const headwordIndex of headwordIndexMap) {
        ...
        if (
          ...
          dictionaryEntry.dictionaryIndex < existing
        ) {
          headwordDictionaryIndices.set(
            headwordIndex,
            dictionaryEntry.dictionaryIndex,
          );
        }
      }
    ...
    for (const { dictionaryEntry, headwordIndexMap } of definitionEntries) {
      ...
      if (dictionaryEntry.isPrimary) {
        isPrimary = true;
        maxOriginalTextLength = Math.max(
          maxOriginalTextLength,
          dictionaryEntry.maxOriginalTextLength,
        );

        const dictionaryEntryInflections =
          dictionaryEntry.inflectionRuleChainCandidates;
        const dictionaryEntryTextProcesses =
          dictionaryEntry.textProcessorRuleChainCandidates;

        if (
          inflections === null ||
          dictionaryEntryInflections.length < inflections.length
        ) {
          inflections = dictionaryEntryInflections;
        }
        if (
          textProcesses === null ||
          dictionaryEntryTextProcesses.length < textProcesses.length
        ) {
    ...
    const { sourceTermExactMatchCount, matchPrimaryReading } =
      this._getHeadwordMatchCounts(headwordsArray, primaryReading);
  ...
  _sortHeadwords(headwordsArray, headwordDictionaryIndices, definitions) {
    ...
    headwordsArray.sort((a, b) => {
      ...
      const aDictIndex =
        headwordDictionaryIndices.get(a.index) ?? Number.MAX_SAFE_INTEGER;
      const bDictIndex =
        headwordDictionaryIndices.get(b.index) ?? Number.MAX_SAFE_INTEGER;
  ...
  _addTermHeadwords(language, headwordsMap, headwords, tagAggregator) {
    ...
    for (const { term, reading, sources, tags, wordClasses } of headwords) {
      ...
      if (typeof headword === "undefined") {
        headword = this._createTermHeadword(
          headwordsMap.size,
          term,
          reading,
          [],
  ...
  _addTermDefinitions(
    ...
    for (const {
      ...
      for (const headwordIndex of headwordIndices) {
        this._addUniqueTermHeadwordIndex(
          newHeadwordIndices,
          headwordIndexMap[headwordIndex],
        );
      }
  ...
  _sortTermDictionaryEntries(dictionaryEntries) {
    ...
    const compareFunction = (v1, v2) => {
      ...
      let i =
        (v2.matchPrimaryReading ? 1 : 0) - (v1.matchPrimaryReading ? 1 : 0);
      if (i !== 0) {
        return i;
      }
      ...
      i =
        this._getShortestTextProcessingChainLength(
          v1.textProcessorRuleChainCandidates,
        ) -
        this._getShortestTextProcessingChainLength(
          v2.textProcessorRuleChainCandidates,
        );
      if (i !== 0) {
        return i;
      }
      ...
      i =
        this._getShortestInflectionChainLength(
          v1.inflectionRuleChainCandidates,
        ) -
        this._getShortestInflectionChainLength(
          v2.inflectionRuleChainCandidates,
        );
      if (i !== 0) {
        return i;
      }
      ...
      for (
        let j = 0, jj = Math.min(headwords1.length, headwords2.length);
        j < jj;
        ++j
      ) {
  ...
  _addUserFacingInflections(language, dictionaryEntries) {
    ...
    for (const dictionaryEntry of dictionaryEntries) {
      ...
      const expandedChains = inflectionRuleChainCandidates.map(
        ({ source, inflectionRules }) => ({
          source,
          inflectionRules:
            this._multiLanguageTransformer.getUserFacingInflectionRules(
              language,
              inflectionRules,
            ),
        }),
